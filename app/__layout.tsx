...
export default function RootLayout() {
  ...
  useEffect(() => {
    fontTimerRef.current = setTimeout(() => setFontTimeout(true), 4000);
    return () => {
  ...
  useEffect(() => {
    if (fontsLoaded || fontError || fontTimeout) {
      ...
      if (authState === "loading") checkAuth();
    }
  ...
  if (authState === "locked") {
    return (
      <View style={styles.lockScreen}>
        ...
        <Text style={styles.lockTitle}>{t("auth.app_locked")}</Text>
        <Text style={styles.lockSubtitle}>{t("auth.authenticate")}</Text>
        <TouchableOpacity style={styles.lockBtn} onPress={authenticate}>
          <Text style={styles.lockBtnText}>{t("auth.retry")}</Text>
        </TouchableOpacity>
      </View>
    );
  }
